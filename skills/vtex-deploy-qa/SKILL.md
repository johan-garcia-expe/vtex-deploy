...
18. Crear PR de registro una vez completado el deploy.
    - Si la feature branch ya fue mergeada a `qa` antes del deploy (flujo qa:release desde branch ya mergeada) → el PR de registro es **innecesario**, omitir este paso.
    Intentar con `gh` CLI:
    ```
    gh pr create \
      --base qa \
    ...
    ```
    Si `gh` no está instalado (command not found) → dar al usuario la URL directa y los datos para crearlo manualmente:
    ```
...
### Fase Git — registro
14. Si la feature branch ya fue mergeada a `qa` antes del deploy → omitir PR de registro (ya está documentado en el merge anterior).
    Si no → crear PR de registro con `gh` CLI o URL directa.
...
- NUNCA ejecutar `vtex switch` sin antes correr `vtex whoami` para verificar la cuenta actual
- NUNCA hacer switch si el usuario ya está en la cuenta correcta
- NUNCA verificar la cuenta solo antes del `vtex release` — verificar ANTES de crear cualquier workspace (dev o producción)
...
- La rama `develop` NO se toca durante el flujo de QA — solo se toca al deployar a Producción
- El PR a `qa` se crea al FINAL del flujo, después del deploy — no es un gate previo
- Si la feature branch ya fue mergeada a `qa` antes del deploy → omitir PR de registro
