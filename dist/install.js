...
function copyCanonicalFiles(destPath, selectedSkills) {
...
function injectOrchestrator(filePath, orchestratorContent, format) {
    ...
    if (format === "json") {
        ...
        if (content.includes(MARKER_START)) {
            injected = content.replace(new RegExp(`${MARKER_START}[\\s\\S]*?${MARKER_END}`), markedBlock);
        }
        else {
...
async function main() {
    ...
    p.log.info(`  Agentes: ${selectedAgents.join(", ")}`);
