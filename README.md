...
| Comando | Descripción |
|---|---|
| `vtex-deploy` | Instala skills y configura el agente en el proyecto |
| `vtex-deploy-init` | Crea `.vtex-deploy.yaml` con interfaz interactiva |
...
Ejecuta en la raíz del proyecto VTEX IO. El instalador detecta qué skills ya están instaladas y solo sobreescribe las que selecciones.

## Configuración del proyecto

Después de instalar, ejecuta el init para crear `.vtex-deploy.yaml`:

```bash
vtex-deploy-init
```

El comando detecta automáticamente el `vendor_prod` desde `manifest.json`, muestra un selector interactivo para elegir las dependencias que deben cambiar al deployar a QA, y mapea las ramas del repositorio.
...
│   ├── vtex-io-app-structure/   — Estructura de apps VTEX IO
│   ├── vtex-manifest/           — Configuración de manifest.json
│   └── vtex-store-framework/    — Store Framework y bloques
...
- **qa:full** — vendor en estado Prod → crea rama `deploy/qa-{fecha}`, transforma vendor, publica, valida y despliega
- **qa:release** — vendor ya en estado QA → publica directamente sin transformar; advierte si está en una feature branch

### vtex-deploy-prod

Flujo de deploy a Producción. Transforma el vendor si es necesario, crea workspace de producción (`-p`), publica y despliega.

### vtex-transform

Transformación de archivos entre vendor QA y Prod:
- `manifest.json` — vendor y dependencias
- Archivos CSS/SCSS — renombra archivos que incluyan el vendor (excepto `_*.*`)
...
Gestión de ramas y PRs. La rama `qa` es one-way — solo recibe PRs desde ramas `deploy/qa-*`, nunca se mergea hacia `develop` o `main`.
...
## Skills de desarrollo VTEX IO

### vtex-io-app-structure

Estructura, builders y configuración de apps VTEX IO.

### vtex-manifest

Campos, dependencias, peerDependencies y policies en `manifest.json`.

### vtex-store-framework

Bloques, templates, layouts, Site Editor, CSS handles y Store Theme.
...
Con el agente configurado, escribe en lenguaje natural:
